...
fn shunting_yard(
    ...
    match token {
        ...
        Token::Assign => {
            return Err(
                "Invalid Token: Assignment must be handled during preprocessor".to_string(),
            );
        }
    }
            ...
            match curr {
                ...
                Token::Identifier(name) => {
                    if let Ok(f) = Function::from(&name) {
                        ...
                        match next {
                            // matches "pi ("
                            Some(Token::LParen) => result.push(Token::Operator(Operator::Mul)),

                            Some(Token::Number(num)) => {
                                return Err(format!(
                                    "Invalid expression: implicit multiplication for {} {} isn't supported",
                                    c, num
                                ));
                            }
                            // matches "pi sin"
