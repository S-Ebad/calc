...
fn factorial(n: f64) -> Result<f64, String> {
  if n < 0.0 || n.fract() != 0.0 {
    return Err(format!("Factorial undefined for {}", n));
  }

  if n > 170.0 {
    return Err(format!("{}! is too large", n));
  }
  ...
  while let Some(token) = tokens.pop_front() {
    match token {
      Token::Operator(Operator::Neg) => match sum_stk.pop() {
        ...
        None => return Err("Negation requires an operand.".to_string()),
      },
      Token::Operator(Operator::Fac) => match sum_stk.pop() {
        Some(num) => sum_stk.push(factorial(num)?),
        None => return Err("Factorial requires an operand".to_string()),
      },

      Token::Operator(op) => {
        let (num2, num1) = match (sum_stk.pop(), sum_stk.pop()) {
          (Some(a), Some(b)) => (a, b),
          _ => return Err("Not enough operands.".to_string()),
        };
      ...
      Token::Function(func) => {
        let arg = match sum_stk.pop() {
          Some(n) => n,
          None => return Err(format!("{:?} requires an argument.", func)),
        };
